<template>
    <div class="flex flex-col bg-page rounded-xl m-4 p-4 space-y-1">
        ...
        <div v-for="day in freeDays" class="flex flex-row space-x-2">
            <div>{{day.date}} </div>
            <button @click="deleteDay(day)" type="button" class="text-right rounded px-2 bg-red-500">Delete</button>
        </div>
        <div>
            <input v-model="freeDayForm.date" type="date" class="md:w-1/6 rounded-md">
            <button @click="addFreeDay" type="button" class="bg-page2 px-3 py-2 rounded-md text-default hover:bg-page3 hover:text-default2 ml-4" :disabled="freeDayForm.processing">Add</button>
        </div>
        <div class=" font-semibold text-red-500" v-if="freeDayForm.errors.date">Error date</div>
        ...
        <div class="z-50 fixed inset-0 w-full h-screen flex items-center justify-center bg-bg-semi-75">
            <div class="md:w-1/3 flex flex-col bg-page3 rounded-xl m-2 p-2">
                ...
                <div class="flex flex-row justify-between mx-auto w-3/4 md:w-1/2">
                    ...
                    <button type="button" class="text-right rounded px-2 bg-page" @click="showDeleteDay = false">Cancel</button>
                </div>
            </div>
        </div>
    </div>
